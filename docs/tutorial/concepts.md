...
!!! note
    ...
    Retroactive changes to a device type are a future feature.
...
Every device in site_manager has to be assigned to a physical location. Locations are managed as a hierarchy.
...
- Racks are the lowest level in the hierarchy. They do not have to represent equipment racks but can also represent other places where devices are mounted like a table or a monitor wall. Devices have to be assigned to a rack. Devices may be assigned to a specific position within the rack if the device type and rack support this.
